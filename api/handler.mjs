...
async function analyzeDigitalPresence(input) {
  ...
  return { score, website, facebook, linkedin, competitors, positioning, coherence, generatedAt: new Date().toISOString() };
}
...
function analyzePositioning(input, website, competitors = []) {
  const ownText = [
    input.company, input.sector, input.location, input.mainOffer,
    input.targetClient, input.commercialMessage,
    website?.title, website?.metaDescription,
    ...(website?.h1 || []), website?.textSample,
  ].join(" ");
  const ownPillars = extractPositioningPillars(input, ownText);
  const readableCompetitors = competitors.filter((c) => c.accessible);
  const competitorProfiles = competitors.map((competitor) => {
    const text = [competitor.title, competitor.metaDescription, ...(competitor.h1 || []), competitor.textSample].join(" ");
    return {
...
function chooseStrongestCompetitor(competitors = []) {
  const readable = competitors.filter((c) => c.accessible);
  if (!readable.length) return null;
  return readable
    .map((c) => ({ ...c, positioningScore: scoreCompetitorPositioning(c) }))
...
function buildPositioningSummary(input, ownPillars, strongestCompetitor, readableCount) {
  const missing = ["cible claire", "offre explicite", "résultat attendu", "zone locale", "différence visible"]
    .filter((p) => !ownPillars.includes(p));
...
function calculateDigitalScore(input, website, facebook, linkedin, coherence) {
  ...
  return { seo, offerClarity, positioning: formPositioning, coherence: coherenceScore, socialActivity, socialEngagement, conversion, global: clamp(global, 0, 100) };
}
